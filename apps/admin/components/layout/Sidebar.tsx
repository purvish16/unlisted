...
const NAV_ITEMS = [
  ...
  { href: '/companies',   icon: 'apartment',        label: 'Companies' },
  { href: '/users',       icon: 'group',            label: 'Users' },
  { href: '/trades',      icon: 'candlestick_chart',label: 'Trades' },
  ...
  return (
    ...
    >
      <div className="px-6 mb-8">
        ...
        <p className="text-[11px] font-bold tracking-[0.05em] uppercase text-error mt-0.5">
          Admin Console
        </p>
      </div>
      ...
      <div className="mt-auto px-3 border-t border-outline-variant/10 pt-3 space-y-0.5">
        <div className="flex items-center gap-3 px-3 py-2">
          <div className="w-7 h-7 rounded-full bg-error/20 flex items-center justify-center text-[11px] font-bold text-error flex-shrink-0">
            {user?.mobile.slice(-2) ?? 'A'}
          </div>
          <div className="flex-1 overflow-hidden">
            <p className="text-[13px] font-semibold text-on-surface truncate">{user?.mobile ?? 'Admin'}</p>
            <p className="text-[11px] text-error font-semibold uppercase tracking-wider">Super Admin</p>
          </div>
        </div>
        <button
