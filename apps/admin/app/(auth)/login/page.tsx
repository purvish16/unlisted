...
export default function AdminLoginPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.user.role !== 'admin') {
        setError('Access denied. This console is for platform administrators only.');
        return;
      }
  ...
  return (
    <div className="bg-surface-container-lowest rounded-2xl shadow-floating p-8">
      <div className="flex items-center gap-2 mb-6 p-3 bg-error/5 rounded-xl">
        <span className="material-symbols-outlined text-error text-[20px]">admin_panel_settings</span>
        <span className="text-[13px] font-semibold text-error">Restricted Access — Admins Only</span>
      </div>

      {step === 'mobile' ? (
        <>
          <h2 className="text-[22px] font-bold text-on-surface mb-1">Admin Sign In</h2>
          <p className="text-[14px] text-outline mb-5">Enter your registered admin mobile number</p>
        ...
        <>
          <button onClick={() => { setStep('mobile'); setOtp(''); setError(''); }} className="flex items-center gap-1.5 text-[13px] text-outline hover:text-primary transition-colors mb-4">
            <span className="material-symbols-outlined text-[18px]">arrow_back</span>
            Change number
          </button>
          <h2 className="text-[22px] font-bold text-on-surface mb-1">Verify OTP</h2>
          <p className="text-[14px] text-outline mb-5">Sent to <span className="font-semibold text-on-surface">+91 {mobile}</span></p>
          ...
          <div className="text-center mt-4">
            {resendTimer > 0
              ? <p className="text-[13px] text-outline">Resend in <span className="font-semibold">{resendTimer}s</span></p>
              : <button onClick={async () => { try { await authApi.sendOtp(mobile); startResendTimer(); } catch { /* ignore */ } }} className="text-[13px] text-primary font-semibold hover:underline">Resend OTP</button>
            }
          </div>
        </>
      )}
