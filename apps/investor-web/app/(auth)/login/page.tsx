  ...
  return (
    <div className="bg-surface-container-lowest rounded-2xl shadow-floating p-8">
      {step === 'mobile' ? (
        <>
          <div className="mb-6">
            <h2 className="text-[22px] font-bold text-on-surface">Welcome back</h2>
          ...
          <div className="text-center mt-4">
            {resendTimer > 0 ? (
              <p className="text-[13px] text-outline">
                Resend OTP in <span className="font-semibold text-on-surface">{resendTimer}s</span>
              </p>
            ) : (
              <button
                onClick={handleResend}
                disabled={loading}
                className="text-[13px] text-primary font-semibold hover:underline disabled:opacity-60"
              >
                Resend OTP
              </button>
            )}
